...
/****        FilePackage    *****/
/********************************/
function FilePackage(file,pubSub,receiving, temp){
	if(temp) console.log('FilePackage called from: '+ temp);
  this.name = file.name;
  this.path = file.path;
...
Delivery.prototype.subscribe = function() {
  ...
  this.pubSub.subscribe('receive.success',function(file){
  	console.log('receive.success, with file.path:');
  ...
  this.socket.on('receive.start',function(file){
  	console.log('receive.start, with file:');
  	console.log(file.path);
  	console.log('\n');
    _this.pubSub.publish('receive.start',file.uid);
    var filePackage = new FilePackage(file,_this.pubSub, true, 'receive.start');
    _this.receiving[file.uid] = filePackage;
  });

  this.pubSub.subscribe('file.load',function(filePackage){
  	console.log('file.load, with filePackage.name:');
  ...
  this.socket.on('delivery.connecting',function(){
  	console.log('delivery.connecting');
  		console.log('\n');
  ...
  this.socket.on('send.start',function(filePackage){
    //receive.start
    console.log('send.start, with filePackage.name:');
  	console.log(filePackage.name);
  	console.log('\n');
    _this.pubSub.publish('receive.start',filePackage);
    var fp = new FilePackage(filePackage, _this.pubSub, true, 'send.start');
    _this.receiving[filePackage.uid] = fp;
  });

  //server sending to client
  this.socket.on('send.success', function(uid){
  	console.log('send.success, with uid:');
  	console.log(uid);
...
Delivery.prototype.send = function(file){
	console.log('SEND(): with file:');
	console.dir(file);
  var filePackage = new FilePackage(file,this.pubSub, null, 'send');
